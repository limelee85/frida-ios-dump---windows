...
function dumpModule(name) {
  ...
  if (magic2 === FAT_CIGAM || magic2 === FAT_MAGIC) {
    let off2 = 4;
    const archs = swap32(getU32(buffer.add(off2)));
    for (let i = 0; i < archs; i++) {
      const cputype = swap32(getU32(buffer.add(off2 + 4)));
      const cpusubtype = swap32(getU32(buffer.add(off2 + 8)));
      if (cur_cpu_type === cputype && cur_cpu_subtype === cpusubtype) {
        fileoffset = swap32(getU32(buffer.add(off2 + 12)));
        filesize = swap32(getU32(buffer.add(off2 + 16)));
        break;
      }
      off2 += 20;
    }
...
function main() {
  const app_path = frida_objc_bridge_default.classes.NSBundle.mainBundle().executablePath().stringByDeletingLastPathComponent();
